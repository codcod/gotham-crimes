...
def get_crimes(db: Session, skip: int = 0, take: int = 100):
    return db.query(Crime).offset(skip).limit(take).all()


def create_crime(db: Session, crime):
    date_time = crime['date_time']
    crime['date_time'] = dt.fromisoformat(date_time)
    # crime['date_time'] = dt.now()
